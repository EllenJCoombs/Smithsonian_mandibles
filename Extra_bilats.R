...
#####################
#
# Create additional midline points for everything 
#
#


LMX_bilat=manual_coords[c(4,22),,] ## LM1 on left (5) and corresponding LM on the right side (69)
LMY_bilat=manual_coords[c(15,29),,] 
LMA_bilat=manual_coords[c(5,23),,] 
LMB_bilat=manual_coords[c(14,28),,] 

LMX_midline=colSums(LMX_bilat)/2
LMY_midline=colSums(LMY_bilat)/2
LMA_midline=colSums(LMA_bilat)/2
LMB_midline=colSums(LMB_bilat)/2

spheres3d(LMX_midline[,15], col = 'red', radius = 0.004)
spheres3d(LMY_midline[,15], col = 'red', radius = 0.004)
spheres3d(LMA_midline[,15], col = 'red', radius = 0.004)
spheres3d(LMB_midline[,15], col = 'red', radius = 0.004)
spheres3d(manual_coords[,,15], col = 'black', radius = 0.004)


Shape_data_with_bilats=abind::abind(manual_coords, LMX_midline, LMY_midline, LMA_midline, LMB_midline, along=1)


text3d(Shape_data_with_bilats[,,3],text = 1:36)


#Pull out the mirror coords 
mirror_coords <- Shape_data_with_bilats[c(1:18, 33, 34, 35, 36),,]

text3d(mirror_coords[,,2], text = c(1:18, 33, 34, 35, 36))
...
midline<-as.integer(c(9, 10, 12, 13, 33, 34, 35, 36)) # LM that are on the midline + parasphenoid curve points + NO patch point
...
spheres3d(mirror_coords[c(9, 10, 12, 13, 19:22),,1],radius=0.004,col='red') #midline

right.lm <- c(23:36) #left.lm +1:lenmatrice
...
atarfa=ply2mesh(file="E:/Smithsonian Postdoc/Year 1/Mandible scans/full mandibles asymmetry test/ASYMM DATA/PTS full mand/ply/Delphinapterus leucas NHM 1933.10.13.4.ply")
shade3d(atarfa,col='white')
spheres3d(MirroredAC[,,15],col= 'red', radius=0.004)
spheres3d(manual_coords[,,15], col = 'green' ,radius=0.004)
spheres3d(MirroredAC[c(9, 10, 12, 13, 19:22),,2], col = 'black' ,radius=0.004)
...
get.col.spectrum <- landvR::procrustes.var.plot(manual_coords[,,88], MirroredAC[,,88], col.val = differences_between_lms[[1]][,1], col = colfunc)
